...
public class OpenGLDrawRect {
    ...
    public static ArrayList<FloatBuffer> drawCenterShowRect(boolean isBackCamera, float width, float height, float roi_ratio) {
        ...
        Log.e("默认画绿框的大小", "rectF===" + rectF + "；left：" + _left + "；right：" + _right + "；top：" + _top + "；bottom：" + _bottom
                + "；delta_x：" + delta_x + "；delta_y：" + delta_y);

        // 4个点分别是// top_left bottom_left bottom_right  top_right
    ...
    public static ArrayList<FloatBuffer> drawMosaicRect(boolean isBackCamara, Rect rect, float width, float height) {
        ...
        if (isBackCamara) {
            _top = 1 - (rect.left / width) * 2;
            _bottom = 1 - (rect.right / width) * 2;

            _left = (rect.top / height) * 2 - 1;
            _right = (rect.bottom / height) * 2 - 1;
        } else {
            _top = (rect.left / width) * 2 - 1;
            _bottom = (rect.right / width) * 2 - 1;
        ...
        FloatBuffer buffer = floatBufferUtil(rectangle);
        ArrayList<FloatBuffer> vertexBuffersOpenGL = new ArrayList<>();
        vertexBuffersOpenGL.add(buffer);
